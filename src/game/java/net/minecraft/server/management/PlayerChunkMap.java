...
public class PlayerChunkMap {
	...
	private final LongObjectMap<PlayerChunkMapEntry> playerInstances = new LongObjectHashMap<PlayerChunkMapEntry>(4096);
	...
	public List<Chunk> getChunkIterator() {
		List<Chunk> chunks = Lists.<Chunk>newArrayList();
		for(int i = 0, j = this.playerInstanceList.size(); i < j; ++i) {
			PlayerChunkMapEntry playerchunkmapentry = this.playerInstanceList.get(i);
			Chunk chunk = playerchunkmapentry.getChunk();
			if(chunk != null) {
				Chunk chunk1 = this.computeNext(playerchunkmapentry, chunk);
				if(chunk1 != null) {
					chunks.add(this.computeNext(playerchunkmapentry, chunk));
				} else {
	...
	protected Chunk computeNext(PlayerChunkMapEntry playerchunkmapentry, Chunk chunk) {
	...
	public void tick() {
		...
		if (i - this.previousTotalWorldTime > 8000L) {
			...
			for (int j = 0, k = this.playerInstanceList.size(); j < k; ++j) {
		...
		if (!this.playersNeedingChunks.isEmpty()) {
			long l = EagRuntime.nanoTime() + 50000000L;
			int k = 49;
