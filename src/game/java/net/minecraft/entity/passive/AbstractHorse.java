...
public abstract class AbstractHorse extends EntityAnimal implements IInventoryChangedListener, IJumpingMount {
	...
	private static final DataParameter<Byte> STATUS = EntityDataManager.<Byte>createKey(13, DataSerializers.BYTE);
	private static final DataParameter<Optional<EaglercraftUUID>> OWNER_UNIQUE_ID = EntityDataManager
			.<Optional<EaglercraftUUID>>createKey(14, DataSerializers.OPTIONAL_UNIQUE_ID);
	...
	public void writeEntityToNBT(NBTTagCompound compound) {
		...
		if (this.getOwnerUniqueId() != null) {
			if (world.isRemote && !SingleplayerServerController.isClientInEaglerSingleplayer()) {
				compound.setString("OwnerUUID", this.getOwnerUniqueId().toString());
			} else {
				compound.setString("Owner", this.getOwnerUniqueId().toString());
			}
		}
