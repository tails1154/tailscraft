...
public class EntityTrackerEntry {
	...
	public void updatePlayerList(List<EntityPlayer> players, int trackingDistanceMax) {
		...
		if (!this.updatedPlayerVisibility || this.trackedEntity.getDistanceSq(this.lastTrackedEntityPosX,
			...
			this.updatePlayerEntities(players, trackingDistanceMax);
		}
	...
	public void updatePlayerEntity(EntityPlayerMP playerMP, int trackingDistanceMax) {
		if (playerMP != this.trackedEntity) {
			if (this.isVisibleTo(playerMP, trackingDistanceMax)) {
				if (!this.trackingPlayers.contains(playerMP)
					...
					playerMP.connection.sendPacket(new SPacketEntityHeadLook(this.trackedEntity, (byte) lastHeadMotion));
					...
					if (this.trackedEntity instanceof EntityLivingBase) {
						EntityEquipmentSlot[] slots = EntityEquipmentSlot._VALUES;
						for (int ii = 0; ii < slots.length; ++ii) {
							EntityEquipmentSlot entityequipmentslot = slots[ii];
	...
	public boolean isVisibleTo(EntityPlayerMP playerMP, int trackingDistanceMax) {
		int i = range > trackingDistanceMax ? trackingDistanceMax : range;
	...
	public void updatePlayerEntities(List<EntityPlayer> players, int trackingDistanceMax) {
		for (int i = 0; i < players.size(); ++i) {
			this.updatePlayerEntity((EntityPlayerMP) players.get(i), trackingDistanceMax);
		}
	}
