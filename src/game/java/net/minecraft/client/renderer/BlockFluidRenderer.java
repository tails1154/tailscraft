...
public class BlockFluidRenderer {
	...
	public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn,
		...
		if (!flag1 && !flag2 && !aboolean[0] && !aboolean[1] && !aboolean[2] && !aboolean[3]) {
			...
			if (flag1) {
				flag3 = true;
				float f12 = (float) BlockLiquid.getSlopeAngle(blockAccess, blockPosIn, material, blockStateIn);
			...
			if (flag2) {
				...
				int i2 = l1 >> 16 & 65535;
				int j2 = l1 & 65535;
				worldRendererIn.pos(d0, d1, d2 + 1.0D).color(0.5F, 0.5F, 0.5F, 1.0F).tex((double) f35, (double) f38)
						.lightmap(i2, j2).endVertex();
				worldRendererIn.pos(d0, d1, d2).color(0.5F, 0.5F, 0.5F, 1.0F).tex((double) f35, (double) f37)
						.lightmap(i2, j2).endVertex();
				worldRendererIn.pos(d0 + 1.0D, d1, d2).color(0.5F, 0.5F, 0.5F, 1.0F).tex((double) f36, (double) f37)
						.lightmap(i2, j2).endVertex();
				worldRendererIn.pos(d0 + 1.0D, d1, d2 + 1.0D).color(0.5F, 0.5F, 0.5F, 1.0F)
