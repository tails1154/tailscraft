...
public class BlockDynamicLiquid extends BlockLiquid {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, EaglercraftRandom rand) {
		...
		if (i > 0) {
			...
			for (int i1 = 0; i1 < facings.length; i1++) {
				EnumFacing enumfacing = facings[i1];
	...
	private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
		int i = 1000;
		EnumFacing[] values = EnumFacing.Plane.HORIZONTAL.facingsArray;
		for (int k = 0; k < values.length; k++) {
			EnumFacing enumfacing = values[k];
