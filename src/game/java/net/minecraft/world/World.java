...
public abstract class World implements IBlockAccess {
	...
	public boolean isBlockLoaded(BlockPos pos, boolean allowEmpty) {
		return this.isChunkLoaded(pos.x >> 4, pos.z >> 4, allowEmpty);
	}
	...
	public Chunk getChunkFromBlockCoordsIfLoaded(BlockPos pos) {
		int x = pos.x >> 4;
		int z = pos.z >> 4;
		return this.chunkProvider.func_191062_e(x, z) ? this.chunkProvider.provideChunk(x, z) : null;
	}
	
	public Chunk getChunkFromChunkCoordsIfLoaded(int chunkX, int chunkZ) {
		return this.chunkProvider.func_191062_e(chunkX, chunkZ) ? this.chunkProvider.provideChunk(chunkX, chunkZ) : null;
	}
					...
					public String call() throws Exception {
						try {
							return HString.format("ID #%d (%s // %s)",
									new Object[] { Integer.valueOf(Block.getIdFromBlock(p_190524_2_)),
											p_190524_2_.getUnlocalizedName(),
											p_190524_2_.getClass().getCanonicalName() });
				...
				try {
					...
					crashreportcategory.setDetail("Source block type", new ICrashReportDetail<String>() {
						public String call() throws Exception {
							try {
								return HString.format("ID #%d (%s // %s)",
										new Object[] { Integer.valueOf(Block.getIdFromBlock(p_190529_2_)),
	...
	public void immediateBlockTick(BlockPos pos, IBlockState state, EaglercraftRandom random) {
		...
		if (lightType == EnumSkyBlock.SKY && this.canSeeSky(pos)) {
			...
			if (k2 >= 15) {
				...
				for (int i1 = 0; i1 < values.length; i1++) {
					EnumFacing enumfacing = values[i1];
	...
	public List<Entity> getEntitiesInAABBexcluding(@Nullable Entity entityIn, AxisAlignedBB boundingBox,
		...
		for (int j3 = j2; j3 <= k2; ++j3) {
			for (int k3 = l2; k3 <= i3; ++k3) {
				Chunk chunk = this.getChunkFromChunkCoordsIfLoaded(j3, k3);
				if (chunk != null) {
	...
	public int countEntities(Class<?> entityType) {
		...
		for (int l = this.loadedEntityList.size(); j < l; ++j) {
	...
	public void loadEntities(Collection<Entity> entityCollection) {
		...
		while (var3.hasNext()) {
			Entity entity4 = (Entity) var3.next();
	 ...
	 */
	public EaglercraftRandom setRandomSeed(int p_72843_1_, int p_72843_2_, int p_72843_3_) {
