...
public class BiomeMesa extends Biome {
	...
	public void genTerrainBlocks(World worldIn, EaglercraftRandom rand, ChunkPrimer chunkPrimerIn, int x, int z,
			double noiseVal) {
		if (this.clayBands == null || this.worldSeed != worldIn.getSeed()) {
			this.generateBands(worldIn.getSeed(), !worldIn.getWorldInfo().isOldEaglercraftRandom());
		}

		if (this.pillarNoise == null || this.pillarRoofNoise == null || this.worldSeed != worldIn.getSeed()) {
			EaglercraftRandom random = new EaglercraftRandom(this.worldSeed);
	...
	private void generateBands(long p_150619_1_, boolean scrambleRNG) {
		this.clayBands = new IBlockState[64];
		Arrays.fill(this.clayBands, HARDENED_CLAY);
		EaglercraftRandom random = new EaglercraftRandom(p_150619_1_, scrambleRNG);
	...
	class Decorator extends BiomeDecorator {
		...
		protected void generateOres(World worldIn, EaglercraftRandom random) {
