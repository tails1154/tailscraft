...
public class ChunkProviderServer implements IChunkProvider {
	...
	private final LongObjectMap<Chunk> id2ChunkMap = new LongObjectHashMap<Chunk>();
	private final List<Chunk> id2ChunkListing = new ArrayList<Chunk>();
	...
	public Collection<Chunk> getLoadedChunks() {
		return Lists.<Chunk>newArrayList(this.id2ChunkListing);
	}
	...
	public void unloadAllChunks() {
		Iterator<Chunk> objectiterator = this.id2ChunkListing.iterator();
	...
	public Chunk loadChunk(int x, int z) {
		...
		if (chunk == null) {
			...
			if (chunk != null) {
				...
				this.id2ChunkListing.add(chunk);
				chunk.onChunkLoad();
		...
		if (chunk == null) {
			...
			try {
				...
				crashreportcategory.addCrashSection("Location",
						HString.format("%d,%d", new Object[] { Integer.valueOf(x), Integer.valueOf(z) }));
	...
	public boolean saveChunks(boolean p_186027_1_) {
		int i = 0;
		List<Chunk> list = Lists.newArrayList(this.id2ChunkListing);
