...
public abstract class MapGenStructure extends MapGenBase {
	...
	protected LongObjectMap<StructureStart> structureMap = new LongObjectHashMap<StructureStart>();
	protected List<StructureStart> structureListing = new ArrayList<StructureStart>();
		...
		if (!this.structureMap.containsKey(ChunkPos.asLong(chunkX, chunkZ))) {
			...
			try {
				if (this.canSpawnStructureAtCoords(chunkX, chunkZ)) {
					...
					this.structureListing.add(structurestart);
				...
				crashreportcategory.addCrashSection("Chunk location",
						HString.format("%d,%d", new Object[] { Integer.valueOf(chunkX), Integer.valueOf(chunkZ) }));
	...
	public synchronized boolean generateStructure(World worldIn, EaglercraftRandom randomIn, ChunkPos chunkCoord) {
		this.initializeStructureData(worldIn);
		int i = (chunkCoord.chunkXPos << 4) + 8;
		int j = (chunkCoord.chunkZPos << 4) + 8;
		boolean flag = false;
		Iterator<StructureStart> objectiterator = this.structureListing.iterator();
	...
	protected static BlockPos func_191069_a(World p_191069_0_, MapGenStructure p_191069_1_, BlockPos p_191069_2_,
		...
		for (EaglercraftRandom random = new EaglercraftRandom(); k <= p_191069_7_; ++k) {
			for (int l = -k; l <= k; ++l) {
				...
				for (int i1 = -k; i1 <= k; ++i1) {
					...
					if (flag || flag1) {
						...
						EaglercraftRandom random1 = p_191069_0_.setRandomSeed(l1, i2, p_191069_5_);
						l1 = l1 * p_191069_3_;
						i2 = i2 * p_191069_3_;
