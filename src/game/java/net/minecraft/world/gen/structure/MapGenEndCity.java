...
public class MapGenEndCity extends MapGenStructure {
	...
	protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
		...
		int k = chunkX / 20;
		int l = chunkZ / 20;
		EaglercraftRandom random = this.worldObj.setRandomSeed(k, l, 10387313);
		k = k * 20;
		l = l * 20;
	...
	private static int func_191070_b(int p_191070_0_, int p_191070_1_, ChunkGeneratorEnd p_191070_2_) {
		EaglercraftRandom random = new EaglercraftRandom((long) (p_191070_0_ + p_191070_1_ * 10387313));
	...
	public static class Start extends StructureStart {
		...
		public Start(World worldIn, ChunkGeneratorEnd chunkProvider, EaglercraftRandom random, int chunkX, int chunkZ) {
			super(chunkX, chunkZ);
			this.create(worldIn, chunkProvider, random, chunkX, chunkZ);
		}

		private void create(World worldIn, ChunkGeneratorEnd chunkProvider, EaglercraftRandom rnd, int chunkX,
				int chunkZ) {
			EaglercraftRandom random = new EaglercraftRandom((long) (chunkX + chunkZ * 10387313));
			Rotation rotation = Rotation.values()[random.nextInt(Rotation.values().length)];
