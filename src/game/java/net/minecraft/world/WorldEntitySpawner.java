...
public final class WorldEntitySpawner {
	...
	public int findChunksForSpawning(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs,
		...
		if (!spawnHostileMobs && !spawnPeacefulMobs) {
			...
			List<EntityPlayer> lst = worldServerIn.playerEntities;
			for (int m = 0, n = lst.size(); m < n; ++m) {
				...
				if (!entityplayer.isSpectator()) {
					...
					int b0 = (byte) worldServerIn.getMinecraftServer().getConfigurationManager().getViewDistance();

					for (int l = -b0; l <= b0; ++l) {
						for (int i1 = -b0; i1 <= b0; ++i1) {
							boolean flag = l == -b0 || l == b0 || i1 == -b0 || i1 == b0;
							ChunkPos chunkcoordintpair = new ChunkPos(l + j, i1 + k);
							if (!this.eligibleChunksForSpawning.contains(chunkcoordintpair)
									&& worldServerIn.getChunkProvider().chunkExists(chunkcoordintpair.chunkXPos,
											...
											if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype,
												...
												try {
													entityliving = (EntityLiving) EntityList.createEntityByClassUnsafe(
															biome$spawnlistentry.entityClass, worldServerIn);
	...
	public static void performWorldGenSpawning(World worldIn, Biome biomeIn, int p_77191_2_, int p_77191_3_,
			int p_77191_4_, int p_77191_5_, EaglercraftRandom randomIn) {
						...
						if (canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType.ON_GROUND, worldIn,
							...
							try {
								entityliving = (EntityLiving) EntityList
										.createEntityByClassUnsafe(biome$spawnlistentry.entityClass, worldIn);
							} catch (Exception exception) {
								exception.printStackTrace();
								continue;
							}
