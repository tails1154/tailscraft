...
public class Teleporter {
	...
	private final LongObjectMap<Teleporter.PortalPosition> destinationCoordinateCache = new LongObjectHashMap<Teleporter.PortalPosition>(
			4096);
	...
	public void removeStalePortalLocations(long worldTime) {
		if (worldTime % 100L == 0L) {
			...
			LongCollection collection = this.destinationCoordinateCache.keys();
			for (LongCursor cursor : collection) {
				long key = cursor.value;
				Teleporter.PortalPosition teleporter$portalposition = destinationCoordinateCache.get(key);

				if (teleporter$portalposition == null || teleporter$portalposition.lastUpdateTime < i) {
					destinationCoordinateCache.remove(key);
				}
			}
		}
	}
