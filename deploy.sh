...
(
	...
	sh ./CompileWASM.sh
	sh ./CompileEPK.sh
	sh ./CompileEagRuntimeJS.sh
)

if [ ! -f "$source_dir/classes.wasm" ] || [ ! -f "$source_dir/assets.epk" ] || [ ! -f "$source_dir/eagruntime.js" ]; then
	printf '%s\n' "Missing compiled client artifacts in $source_dir" >&2
...
cleanup() { rm -f "$deploy_dir/classes.wasm.new" "$deploy_dir/assets.epk.new" "$deploy_dir/eagruntime.js.new"; }
trap cleanup EXIT HUP INT TERM

cp "$source_dir/classes.wasm" "$deploy_dir/classes.wasm.new"
cp "$source_dir/assets.epk" "$deploy_dir/assets.epk.new"
cp "$source_dir/eagruntime.js" "$deploy_dir/eagruntime.js.new"
mv "$deploy_dir/classes.wasm.new" "$deploy_dir/classes.wasm"
mv "$deploy_dir/assets.epk.new" "$deploy_dir/assets.epk"
mv "$deploy_dir/eagruntime.js.new" "$deploy_dir/eagruntime.js"
cmp "$source_dir/classes.wasm" "$deploy_dir/classes.wasm"
cmp "$source_dir/assets.epk" "$deploy_dir/assets.epk"
cmp "$source_dir/eagruntime.js" "$deploy_dir/eagruntime.js"

printf '%s\n' "Deployed classes.wasm, assets.epk, and eagruntime.js to: $deploy_dir"
printf '%s\n' "Backup created at: $backup_dir"
sha256sum "$deploy_dir/classes.wasm" "$deploy_dir/assets.epk" "$deploy_dir/eagruntime.js"
